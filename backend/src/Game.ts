...
export class Game {
  ...
  constructor(id: string, player1: Player,gametype:GameType) {
    ...
    if(gametype=="Blitz"){
      this.timer=new Timer(id , 180, 180,'w',(gameId:string,loser:string)=>this.ontimeOut(loser),(whiteTimer:number , blackTimer:number)=>this.gameTick(whiteTimer,blackTimer))
    }else if (gametype=="Bullet"){
      this.timer=new Timer(id , 60, 60,'w',(gameId:string,loser:string)=>this.ontimeOut(loser),(whiteTimer:number , blackTimer:number)=>this.gameTick(whiteTimer,blackTimer))
    }else{
